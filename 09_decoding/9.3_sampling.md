...
**Min-p 采样**把截断阈值绑定到当前最可能词元的概率上：若最高概率为 $$p_{\max}$$，则只保留满足 $$P(x_i) \geq \alpha p_{\max}$$ 的词元，其中 $$\alpha$$ 是较小的比例阈值。它与 Top-p 的差别在于：Top-p 固定累计概率预算，而 Min-p 根据当前分布的峰值动态收缩或放宽候选集合，在高温采样时常比固定 Top-k/Top-p 更稳。
...
vLLM 主仓库中的 [`vllm/v1/sample/ops/topk_topp_sampler.py`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/sample/ops/topk_topp_sampler.py) 采用了一个数学上等价、但更利于硬件执行的形式：
