...
Top-p 采样因此在多样性和质量之间取得了更好的平衡，是当前大语言模型最常用的采样策略——但它花的是一份固定的**累积概率预算**，分布本身被拉平时（例如高温采样），同样的 $$p$$ 会把更多低概率词元买进候选集。
...
前面的策略都在回答“如何从分布中选词”；实际 API 中还有一类与温度、Top-p 并列的常用参数，直接**修改 logits 本身**来抑制复读。

**重复惩罚**（Repetition Penalty）源自 [CTRL 论文](https://arxiv.org/abs/1909.05858)的 penalized sampling，主流实现（如 Hugging Face transformers）的形式是：设惩罚系数 $$\theta > 1$$，对已出现过词元的 logit $$z_i$$，若 $$z_i > 0$$ 则除以 $$\theta$$、若 $$z_i < 0$$ 则乘以 $$\theta$$——分符号处理保证无论 logit 正负，惩罚都朝降低概率的方向起作用。$$\theta$$ 常取 $$1.1 \sim 1.3$$，过大会让模型刻意回避标点、代词等正常高频词。

OpenAI 风格的 API 则提供两个**加性**变体：**频次惩罚**（frequency penalty）按词元已出现的次数成比例地从 logit 中扣减，重复越多惩罚越重；**存在惩罚**（presence penalty）只要词元出现过就扣减固定值，鼓励引入新话题而不计较重复次数。三者都作用于 Softmax 之前，可与温度和截断策略自由组合。

### 9.3.6 策略的组合使用

在实际应用中，这些策略通常**组合使用**：先用温度调整分布，再用 Top-p 或 Top-k 截断尾部，最后从截断后的分布中采样。例如，典型的对话模型配置为 $$T = 0.7$$、$$\text{Top-p} = 0.9$$。

### 9.3.7 Gumbel-Max 采样：GPU 友好的等价方案

前面介绍的采样策略都需要从概率分布中按权重抽取下一个词元。最直接的实现是**多项式采样**（multinomial sampling）：生成均匀随机数 $$u \sim \text{Uniform}(0, 1)$$，对概率数组做前缀和（cumulative sum），返回第一个累积概率超过 $$u$$ 的位置。这种方法逻辑直观，但并不贴合 GPU 的执行模型：前缀和虽有高效的并行扫描（parallel scan）实现，整个流程仍是“扫描 + 查找”多趟**相互依赖**的 kernel 与同步（PyTorch 的 `torch.multinomial` 还会因此引入 CPU-GPU 同步）；对动辄 128K~256K 大小的现代词表（如 Llama 3 的 128,256 词表），这不如纯 element-wise 运算加一次 argmax 来得高效。
...
vLLM 主仓库 [`vllm/v1/sample/ops/topk_topp_sampler.py`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/sample/ops/topk_topp_sampler.py) 采用了一个数学上等价、但更利于硬件执行的形式：`forward_native` 先把 logits 归一化为概率，再由 `random_sample` 注入指数噪声并取 argmax。剥离 per-request generator 分支与末尾纯展平的 `.view(-1)` 等工程细节后，这条核心路径可提炼为如下片段：

```python
def sample_with_gumbel_max(logits):
...
完全避开了多项式采样“前缀和 + 查找”的多级依赖与同步。若词表沿**张量并行**维度切分，裸的全词表分类采样可以用“每卡局部最大值 + 全局 max-with-index 规约”的方式减少跨卡同步；但在 Top-k / Top-p、温度、禁词表、结构化约束等生产路径中，仍可能需要全局候选集合、概率质量或约束状态的协同处理，实际通信收益取决于推理框架的具体实现。
