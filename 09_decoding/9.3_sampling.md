...
完全避开了多项式采样“前缀和 + 查找”的多级依赖与同步。这在词表沿**张量并行**维度切分到多张 GPU 的场景下尤为重要：传统多项式采样需要 all-gather 整张 logits 或跨卡前缀和加二分查找，通信量为 $$O(V)$$；而 Gumbel-Max 形式只需在每卡本地算出局部最大值与索引，再做一次 `All-Reduce(MAX with index)`，通信量降到 $$O(\text{world\_size})$$。
