...
至此，第二部分“训练篇”结束。本章解决了**如何把人类偏好写进模型的权重**——SFT 换数据、RLHF 换奖励信号、DPO 换损失、LoRA 换可训练的参数；但它留下一个没回答的问题：全章都把“生成”当成现成的一步（8.2 里 rollout 直接交给推理引擎），从未说明对齐后的权重每一步给出的词元概率，究竟该怎么变成用户看到的那句话。
