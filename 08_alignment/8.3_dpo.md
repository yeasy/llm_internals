...
**第一步：Bradley-Terry 偏好模型。** RLHF 假设人类偏好服从 Bradley-Terry 模型——即给定两个回答 $$y_1$$ 和 $$y_2$$，人类偏好 $$y_1$$ 的概率由奖励函数 $$r$$ 决定：

$$P(y_1 \succ y_2 | x) = \sigma(r(x, y_1) - r(x, y_2))$$

其中 $$\sigma$$ 是 sigmoid 函数。直觉上，奖励差越大，偏好概率越高。

**第二步：KL 约束下的最优策略。** RLHF 的目标是在 KL 散度约束下最大化期望奖励：$$\max_{\pi} \mathbb{E}_{y \sim \pi}[r(x, y)] - \beta \text{KL}[\pi || \pi_{\text{ref}}]$$。这个约束优化问题有解析解——最优策略为：

$$\pi^*(y|x) = \frac{1}{Z(x)} \pi_{\text{ref}}(y|x) \exp\left(\frac{r(x,y)}{\beta}\right)$$

反解奖励函数可得：$$r(x, y) = \beta \log \frac{\pi^*(y|x)}{\pi_{\text{ref}}(y|x)} + \beta \log Z(x)$$。

**第三步：代入消除奖励模型。** 将上式代入 Bradley-Terry 模型，由于 $$Z(x)$$ 在两个回答的奖励差中相消，最终得到 DPO 损失——直接用策略模型的对数概率比替代了奖励模型：
...
- **GRPO**（Group Relative Policy Optimization）：DeepSeek 等模型采用的方法，使用组内标准化优势替代单独的价值网络，公式见 [14.6 节](../14_future_trends/14.6_test_time_scaling.md)
