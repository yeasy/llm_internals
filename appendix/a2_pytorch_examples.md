...
```python
def rope_frequencies(d, max_len, base=10000.0):
    """预计算各位置、各二维子空间的旋转角度表"""
    i = torch.arange(0, d, 2, dtype=torch.float32)   # 0, 2, ..., d-2
    theta = base ** (-i / d)                         # (d/2,)，即 10000^(-2i/d)
    m = torch.arange(max_len, dtype=torch.float32)   # 位置 0, 1, ..., max_len-1
