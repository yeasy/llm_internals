...
本章解决了注意力内部的算法问题：三重投影如何分工、$$\sqrt{d_k}$$ 为何必要、多头如何切分子空间、因果掩码如何限制信息流向、$$O(n^2)$$ 的代价换回了什么。但它留下一个没回答的问题——本章所有公式都从 $$Q = XW^Q$$ 里的 $$X \in \mathbb{R}^{n \times d_{\text{model}}}$$ 出发，而 $$n$$ 由谁决定、$$X$$ 的每一行从哪里来，本章始终没有交代。
