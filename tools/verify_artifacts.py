...
def summary_mermaid_count(root: Path) -> int:
    ...
    for match in re.finditer(
        ...
        seen.add(path)
        total += len(
            re.findall(r"(?m)^\s*```mermaid\s*$", path.read_text(encoding="utf-8"))
        )
    return total
...
def verify_html(path: Path, title: str, expected_mermaid_count: int) -> None:
    ...
    placeholder = re.search(
        r"MERMAIDZZ\d+ZZ|PGBKZZ|<pre\b[^>]*\bdiagram-fallback\b", text, re.I
    )
    if placeholder:
    ...
    if len(figures) != expected_mermaid_count or any(
        "<svg" not in item.lower() for item in figures
    ):
