...
本章解决了 Transformer 从何而来的问题：串行瓶颈、信息瓶颈与有限感受野如何一步步逼出“完全基于注意力”的架构决策。但它留下一个没回答的问题——[1.3 节](1.3_attention_birth.md)计算对齐分数用的是解码器状态 $$s_t$$ 与编码器隐藏状态 $$h_j$$，二者都由 RNN 逐个时间步产生，而 $$h_j$$ 既是算分时的比对依据、又是加权求和时被取走的内容；循环被整个拿掉之后，这两个角色由谁来扮演、又该不该拆开？
