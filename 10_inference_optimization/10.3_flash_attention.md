...
由此 softmax 输出可以恢复为 $$P_i = e^{S_i - \text{LSE}}$$——把"减最大值再除以分母"两步合并成一次"减 LSE 再取指数"，化除法为减法，且每行只需写回 1 个 FP32 标量，HBM 写带宽减半。LSE 是后续几个关键机制的共同基石：反向传播时无需保留完整的 $$P$$ 矩阵即可精准重算、Flash-Decoding 的 Split-K 局部归约可以无损合并、Ring Attention 的跨卡 K/V 流转可以增量更新输出（见 [10.3.5](#10.3.5)）。
...
| 版本 | 目标架构 | 关键新原语 |
|:-----|:---------|:-----------|
| FA1 | SM70-80 (Volta–Ampere) | `mma.sync` 矩阵乘 |
| FA2 | SM80 (Ampere) | `cp.async` 异步加载 |
| FA3 | SM90 (Hopper) | TMA、WGMMA、Warp Group 专门化 |
| FA4 | SM100-110 (Blackwell) | TMEM、`tcgen05.mma`、更细粒度异步 |
