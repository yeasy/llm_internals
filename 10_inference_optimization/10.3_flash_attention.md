...
图 10-2：Flash Attention 3 在 Hopper 上的分层内存与异步数据路径
...
Flash Attention 3 的演进揭示了一个重要趋势：**算法设计必须与硬件架构协同演化**。每一代新硬件都会引入新的计算原语和内存层级，只有深入理解这些硬件特性，才能充分释放算力。这一思想在 Blackwell 架构的 FP4 支持中得到了进一步延伸（详见 [11.3 节](../11_serving/11.3_disaggregated_serving.md)）。

### 10.3.5 序列并行：超长上下文的分布式注意力

当单张 GPU 的显存无法容纳超长序列的 KV 缓存时（如处理 1M+ Token 的上下文），需要采用**序列并行**（Sequence Parallelism）将注意力计算分散到多张 GPU 上。

**Ring Attention**（Liu et al., 2024）是一种优雅的序列并行方案：
- 将序列 Q、K、V 在序列维度分割，分配给不同 GPU
- 每个 GPU 只持有部分序列的 KV 缓存，计算对应部分的注意力贡献
- 通过**环形拓扑**异步传输注意力中间结果，使得 GPU 间只需进行一次 All-Reduce 操作
- 通信复杂度从注意力机制的 $$O(n^2)$$ 降至 $$O(n)$$

相比张量并行（Tensor Parallelism）或流水线并行在长序列推理中的通信开销，Ring Attention 通过序列维度的分割提高了通信效率，使得在高速网络（InfiniBand、RoCEv2）环境下，可以以可接受的延迟支持 1M+ Token 推理。这在文档分析、长篇内容理解等场景中至关重要。
