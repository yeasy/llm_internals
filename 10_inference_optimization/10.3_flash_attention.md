...
标准实现分三步，每一步都把一个 `[N, N]` 的矩阵写进显存（HBM），下一步再读回来：

1. `Q[N, d] × Kᵀ[d, N] → S[N, N]`，写 S。
2. 读 S，逐行 Softmax 得 `P[N, N]`，写 P。
3. 读 P，`P[N, N] × V[N, d] → O[N, d]`，写 O。

**这张矩阵有多大。** `N = 8,192`、32 个头、每头 128 维、FP16 时，单层的 S 为 `8,192² × 32 × 2 B = 4 GiB`，P 同样大。同一层的 Q 只有 `8,192 × 32 × 128 × 2 B = 64 MiB`，K、V、O 同。`N = 32,768` 时 S 是 64 GiB，单卡放不下。训练时 P 还要留到反向传播。
...
[FlashAttention](https://arxiv.org/abs/2205.14135)（Dao 等，2022）用两个办法让 `[N, N]` 的矩阵不落显存。**分块计算**（tiling）：把 Q、K、V 切成能放进 SRAM 的小块，逐对块在片上算完。**重计算**（recomputation）：前向不保存 P，反向时按块重算。结果与标准注意力逐位等价，不是近似。

**分块前向。** 下面按 FlashAttention-2 的循环次序写出单个头的前向，方括号里标出每个量驻留的位置。

```text
输入：Q、K、V [N, d]，驻留 HBM；块大小 B_r（行）、B_c（列）
把 Q 切成 T_r = ⌈N / B_r⌉ 个行块，把 K、V 各切成 T_c = ⌈N / B_c⌉ 个列块
for i = 1 .. T_r:                               # 外层：Q 的行块
    读入 Q_i [B_r, d]                            # HBM → SRAM
    初始化 m = −∞ [B_r]，ℓ = 0 [B_r]，Õ = 0 [B_r, d]   # SRAM
    for j = 1 .. T_c:                           # 内层：K/V 的列块
        若第 j 块整块被因果掩码遮住：跳过
        读入 K_j、V_j [B_c, d]                   # HBM → SRAM
        S = Q_i × K_jᵀ / √d                      # [B_r, B_c]，只在 SRAM
        m_new = max(m, rowmax(S))
        ℓ = ℓ · e^(m − m_new) + rowsum(e^(S − m_new))
        Õ = Õ · e^(m − m_new) + e^(S − m_new) × V_j
        m = m_new
    写回 O_i = Õ / ℓ [B_r, d]，LSE_i = m + ln ℓ [B_r]   # SRAM → HBM
```
...
**块多大。** 设 SRAM 能放 M 个数；FA1 论文称 M 的典型值约 100 KB，下面取 `M = 10⁵`。片上要同时放下 Q、K、V、O 各一块，每块约 `B × d` 个数，所以 FA1 取 $$B_c = \lceil M/(4d) \rceil$$、$$B_r = \min(B_c, d)$$。`d = 64` 时，$$B_c = 391$$，$$B_r = 64$$；分数块 `[64, 391]` 只有约 2.5 万个数。

**省了多少访存。** FA1 论文的定理 2 给出 HBM 访问量：标准注意力为 $$\Theta(Nd + N^{2})$$，FlashAttention 为 $$\Theta(N^{2}d^{2}/M)$$。直观推导：每个列块约 M 个数，K、V 共切成 $$\Theta(Nd/M)$$ 块；每处理一块要把 Q 和 O 过一遍，读写 $$\Theta(Nd)$$ 个数；相乘得 $$\Theta(N^{2}d^{2}/M)$$。与 $$N^{2}$$ 相比少了约 $$M/d^{2}$$ 倍：`d = 64`、`M = 10⁵` 时约 24 倍，`d = 128` 时约 6 倍。论文还证明，M 在 d 与 Nd 之间时，任何精确注意力算法都无法在渐近意义上做得更好。
...
标准实现为反向保存 `P[N, N]`。FlashAttention 前向只保存 `O[N, d]` 和每行一个 LSE。`N = 8,192`、32 头时，LSE（FP32）共 `8,192 × 32 × 4 B = 1 MiB`，对比 P 的 4 GiB，激活显存从 $$O(N^{2})$$ 降到 $$O(N)$$。反向时再按同样的分块读入 $$Q_i$$、$$K_j$$、$$V_j$$，在片上重算 $$S_{ij}$$ 与 $$P_{ij} = e^{S_{ij} - \text{LSE}_i}$$，用完即弃。代价是多做一遍前向的注意力 FLOPs，即表 10-8 里多出的 13%；换来的是不读写 `[N, N]` 的矩阵，总时间更短。这与 [7.5 节](../07_distributed_training/7.5_activation_checkpointing.md)的激活检查点是同一种取舍，只是粒度在内核内部。
...
**Ring Attention**（[Liu 等，2023](https://arxiv.org/abs/2310.01889)）是 10.3.2 的分块循环的跨卡版本：

- Q、K、V 沿序列维切成 P 段，第 p 张卡常驻第 p 段。
- 内层循环的“读入下一个 K/V 块”改成从环上的前一张卡接收；每张卡算完当前块，把手里的 K/V 块传给下一张卡。P − 1 轮后，每段 Q 都见过全部 K/V。
- 各轮的部分结果用 LSE 合并（10.3.2），与单卡结果逐位等价。
- K/V 块的传输与块内注意力计算重叠进行。

**通信量。** 每层每轮，每张卡发送一个 K/V 块，字节数为 $$2 \times (N/P) \times H_{kv} \times d_h \times s$$。Llama 3 8B、`N = 10⁶`、`P = 8`、BF16 时为 `2 × 125,000 × 8 × 128 × 2 = 512 MB`，每层传 7 轮。GQA 把这笔通信也缩小到 MHA 的 1/4。块内计算时间须不短于传输时间，通信才能被完全隐藏；序列越长，每块的计算量按 $$(N/P)^{2}$$ 增长，传输量只按 $$N/P$$ 增长，越容易满足。

**因果掩码下的负载不均。** 连续切分时，第 1 张卡的 Q 只需要看第 1 段 K/V，其余各轮收到的块整块被掩码；第 P 张卡每轮都有活。以“块对”计，各卡的工作量是 0.5、1.5、……、P − 0.5，`P = 8` 时最重与最轻相差 15 倍，整体进度由最慢的卡决定。[Striped Attention](https://arxiv.org/abs/2311.09431) 改为让每张卡持有均匀散布在序列各处的词元，各卡每轮的工作量相等；论文报告在 256k 序列的因果训练上，端到端吞吐最高为原始 Ring Attention 的 1.45 倍。把序列切成 2P 段、首尾配对分给各卡，是同一目的的另一种做法。

**另一条路线：按头切分。** [DeepSpeed-Ulysses](https://arxiv.org/abs/2309.14509) 在注意力前后各做一次 all-to-all：进入注意力前，把“每卡一段序列、全部头”换成“每卡全部序列、一部分头”；各卡用普通的 FlashAttention 算自己的头，再换回来。论文的通信分析是：隐藏宽度为 h 时，每层两次 all-to-all 的每链路通信量合计 `4Nh/P`，N 与 P 同比例增大时保持不变。每张卡分到互不重叠的一部分头，所以并行度不能超过头数。Ring Attention 没有这个上限，但要专门的内核和负载均衡。
...
Decode 时查询长度为 1，分数只有一行 `[1, t]`，不存在 `[N, N]` 的矩阵，10.3.1 的问题并不出现。此时的瓶颈是把长度为 t 的 K、V 从显存读一遍（10.1.2），FlashAttention 省 IO 的收益小得多。内核的优化点随之不同：
...
- 序列很短时，`[N, N]` 本来就小，分块的收益有限；收益随 N 增大而增大。
...
- 自定义的注意力偏置或掩码若不在内核支持的形态内，会退回标准实现，`[N, N]` 重新落到显存。
