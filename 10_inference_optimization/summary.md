...
**KV 缓存**避免了重复计算之前词元的 Key/Value，**GQA** 通过让多个查询头共享 KV 头来减小缓存大小。**PagedAttention** 借鉴虚拟内存管理降低显存碎片化，**MLA** 进一步将 KV 压缩为低维隐向量。**FlashAttention** 通过 IO 感知的分块算法避免在 HBM 中存储完整的 $$n \times n$$ 注意力矩阵，并在 **Ring Attention** 中支持分布式超长序列推理。
...
本章解决了每一项优化各自为什么有效，但留下一个没回答的问题：它们彼此是相互牵制的——量化 KV 缓存可以缓解瓶颈，增大批量却会吃掉投机验证所要的空闲算力。这类取舍不落在任何单项技术内部，谁来仲裁？这正是下一章的起点。
