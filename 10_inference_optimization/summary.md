...
**KV 缓存**用显存换掉了重复计算，每词元占 $$2Ln_{kv}d_hb_{\text{elem}}$$ 字节（$$b_{\text{elem}}$$ 为每个数的字节数）。**GQA** 减少头数；**MLA** 把每层每词元的 K、V 换成 512 维隐向量加 64 维各头共用的位置键，共 576 个数；CSA/HCA 压缩词元数量；跨层共享减少层数；KV 量化减少每个数的字节。**PagedAttention** 把碎片与过度预留的浪费压到最后一块以内。**前缀缓存**只能复用从头开始的相同前缀。

**FlashAttention** 用分块、在线 Softmax 和反向重计算，让 $$T \times T$$ 的注意力矩阵不落显存：FLOPs 略增，HBM 读写降到约 1/9，结果逐位等价。FA2 到 FA4 的算法骨架不变，变的是循环怎样映射到每一代硬件上；**Ring Attention** 是同一循环的跨卡版本。Decode 时不存在这张矩阵，收益小得多。
