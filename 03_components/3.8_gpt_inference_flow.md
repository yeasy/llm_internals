...
```mermaid
flowchart LR
    chat["聊天模板"] --> tok["分词器<br/>词元 t₁ … tₘ"]
    tok --> repr["词嵌入与位置方案"]
    repr --> prefill["预填充 Prefill<br/>整个 Prompt 通过 N 层"]
    prefill --> first["LM head → logits<br/>选择首词元 y₁"]
    first --> decode["生成 Decode<br/>一个新词元通过 N 层"]
    decode --> next["LM head → logits<br/>选择下一词元"]
    next --> stop{"EOS 或停止条件？"}
    stop -->|"否"| decode
    stop -->|"是"| answer["输出文本"]
```
