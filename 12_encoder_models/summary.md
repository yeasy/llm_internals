...
虽然编码器模型在特定理解任务上仍有优势，但仅解码器的大语言模型已在大多数场景中展现出同等甚至更强的能力。本章解决了编码器一侧的问题——双向预训练怎么做、训练配方与参数量怎么更划算、512 词元的长度上限怎么撑开；但留下一个没回答的问题：既然“没有因果掩码、每个位置都能同时关注前后文”被认定为编码器的核心优势，为什么主流最终倒向了只能从左往右看的解码器？这正是下一章的起点。
