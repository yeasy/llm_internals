...
想象一个二维平面上的向量。将它旋转一个弧度角 $$\theta$$，就得到了一个新的向量。关键观察是：**两个被不同角度旋转的向量之间的点积，只取决于它们旋转角度的差值，而非各自的绝对角度。**

数学上，如果向量 $$q$$ 被旋转 $$m\theta$$ 弧度（位置 $$m$$），向量 $$k$$ 被旋转 $$n\theta$$ 弧度（位置 $$n$$），那么：

$$\bigl(R(m\theta) q\bigr)^T \bigl(R(n\theta) k\bigr) = q^T R((n-m)\theta) k$$

其中 $$R(\alpha)$$ 是下文采用的列向量旋转矩阵。结果**只依赖于 $$(n-m)$$，即两个位置的相对距离**。有些实现或论文会把同一关系写成 $$R((m-n)\theta)$$，差别来自旋转矩阵正负号或左右乘约定；关键是全文公式和代码必须使用同一约定。
...
**天然的相对位置编码。** 注意力分数 $$q_m^T k_n$$ 在经过旋转后，自动变为相对位置 $$(n-m)$$ 的函数。模型不需要学习从绝对位置推导相对位置——这种关系直接嵌入在了数学结构中。

**长度外推的潜力。** 由于旋转操作对任意位置值都有定义，RoPE 理论上适用于任意序列长度。配合长度外推技术（如位置内插、NTK 感知缩放、YaRN 等），RoPE 能够从较短的训练长度外推到数倍甚至数十倍的推理长度。

**相对距离相关的相位偏置。** RoPE 让注意力分数显式依赖相对距离：距离改变时，不同频率分量的相位差也随之改变。论文中讨论的远距离依赖衰减应理解为统计或包络意义上的倾向，而不是任意一对随机向量的点积都会随距离整体或单调下降。

下面的代码直观展示相对距离如何通过多频率相位影响点积——固定一个查询向量，依次计算它与不同相对距离处的键的注意力分数：
...
plt.tight_layout()
plt.savefig("_images/rope_decay.png", dpi=150)
plt.show()
```

![RoPE 注意力分数随相对距离的相位变化示意图](_images/rope_decay.png)

图 4-2：RoPE 注意力分数随相对距离的相位变化示例

从图中可以观察到，注意力分数会随着相对距离变化而振荡，因为不同频率分量的相位差在改变。这条随机曲线不应被解读为单调或必然衰减；更准确的说法是，RoPE 引入了相对距离相关的相位偏置，在统计或包络意义上可能形成远距离依赖减弱的倾向。
...
**NTK 感知缩放**：不是均匀地压缩所有频率，而是更多地缩放低频分量，尽量保留高频分量（神经网络难以重新学习高频位置信息，必须予以保留）。

**YaRN**（Yet another RoPE extensioN）：结合了频率缩放和注意力温度调整，在多种长度外推基准上表现出色，并被一些 LLaMA 系长上下文扩展模型采用。Llama 3.1 等模型还使用了独立的高低频 RoPE 缩放方案，不应直接等同于 YaRN。
