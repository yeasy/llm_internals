...
至此，第一部分“基础篇”结束。读者已经建立了对 Transformer 架构的完整理解——从为什么需要它、它的核心注意力机制、各组件的设计逻辑，到位置编码的演进。本章解决了位置信息该如何注入，但留下一个没回答的问题：全章把每种方案的成败都押在“训练时见过的长度”上——可学习编码在 512 之外无向量可取，RoPE 的四种外推手段都在把超出范围的位置往回掰——而“训练”本身用的是什么目标、要吃掉多少数据，本章一次也没有打开。
