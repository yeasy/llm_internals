...
本章解决了“当代主流 LLM 长什么样”——从 GPT 的规模扩展，到 Llama 确立的标准配方，再到 DeepSeek、Gemini 各自的路线；但留下一个没回答的问题：全章是按模型点名的，技术只跟着模型出场——MoE 凭什么只激活一部分参数就够用，本章给的是结论；百万级上下文靠什么撑起来，本章只点到平方复杂度、KV 缓存与跨设备通信三个瓶颈为止。这正是下一章的起点。
