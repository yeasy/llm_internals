...
对于推理场景，**显存带宽**通常比算力更重要（[10.1 节](../10_inference_optimization/10.1_bottleneck.md)已解释生成阶段是访存密集型的）。H200 的 141 GB HBM3e 显存和 4.8 TB/s 带宽相比 H100（80 GB、3.35 TB/s）提升了 **+45% 推理吞吐**，使其可以在单卡上运行 70B 的 INT4 量化模型。这使 H200 成为中等规模模型部署的首选——显存充足避免多卡张量并行的通信开销，带宽提升直接加速生成阶段。

而 B200 的 192 GB 显存和 8 TB/s 带宽则进一步扩展了单卡承载的模型规模。最新的 B300 更是将显存提升至 288 GB，配合 12 TB/s 的带宽，可在单卡上运行顶级大模型（如 405B）的 INT8 量化版本，显著简化部署拓扑。
...
- **第六代 Tensor Core**：进一步优化 FP4/FP6 等极低精度格式，预期单精度计算性能相比 Blackwell 提升 50%+
- **光电混合互连**：在数据中心级别采用硅光互连（Silicon Photonics），实现 Pb/s 级带宽，相比 NVLink 5.0 的 1.8TB/s 提升两个数量级，使得全球范围的 All-Reduce 通信不再是瓶颈

此外，Google Gemini 项目和 xAI Colossus 的部署实践证明了**光电混合交换网络**在万级 GPU 集群中的可行性：xAI Colossus（200K H100）采用 RoCEv2 以太网而非 InfiniBand，通过优化的网络协议栈（DCQCN）实现了与 InfiniBand 相当的 All-Reduce 性能，同时带来更高的成本效益。这标志着 LLM 推理基础设施从"高端专用网络"向"通用高速以太网"的转变，降低了超大规模部署的技术门槛。
