...
权重显存可以先用 $$\text{参数量(B)}\times\text{位宽(bit)}/8$$ 得到十进制 GB 下限。下面的“可放入”只比较权重与标称显存，便于审计数量级；它**不代表可服务**，因为实际部署还必须给 KV cache、runtime workspace、通信 buffer、量化 scale、并发和碎片留余量。
...
表 11-3：仅权重的十进制 GB 重算与理论单卡容纳关系。边界接近或刚好“是”的组合仍应按服务峰值做实测，并预留安全余量。

对于推理场景，**显存带宽**通常比算力更重要（[10.1 节](../10_inference_optimization/10.1_bottleneck.md)已解释生成阶段是访存密集型的）。H200 的 141 GB HBM3e 显存和 4.8 TB/s 带宽相比 H100（80 GB、3.35 TB/s）带宽提高约 **43%**（NVIDIA 官方基准下这转化为 Llama 2 70B 推理吞吐最高 1.9 倍、GPT-3 175B 约 1.6 倍），使其可以在单卡上以 FP8 运行 70B 模型并为 KV 缓存留出充足空间。70B 模型的 FP8 权重本身约 70 GB，实际还需 KV 缓存、runtime workspace、通信 buffer 和并发余量；H100 80 GB 通常非常紧张，H200 更适合保留服务余量。INT4 量化的 70B 权重约 35 GB，H100 单卡更容易承载。这使 H200 成为中等规模模型部署的重要选择——显存充足可减少多卡张量并行的通信开销，带宽提升直接加速生成阶段。
...
**TPU**（Tensor Processing Unit）是 Google 专门为深度学习设计的加速器。**TPU v6e（Trillium）** 相比 v5e 在每芯片计算性能上提升了 4.7 倍，HBM 容量和带宽均大幅增加，并支持 FP8 等低精度格式。Google 于 2026 年 4 月公开第八代 TPU：**TPU 8t** 面向大规模训练，配备 216 GB HBM、6528 GB/s 带宽；**TPU 8i** 面向低延迟推理与推理型负载，配备 288 GB HBM、8601 GB/s 带宽。两者虽已发布，但 Google Cloud TPU 当前产品页仍标注 **Coming soon**，不能写成客户已普遍可申请；作为对照，Ironwood TPU7x 已在 2026 年 3 月 31 日 GA。
...
1. **模型大小与显存**：决定了最低显存需求。70B 模型 FP8 权重约 70 GB，但服务化部署还要为 KV cache、workspace、并发和碎片预留显存，通常不应按权重大小贴边规划；140B+ 模型通常需要 H200/B200/B300 或多卡方案。单卡无法容纳则进行张量并行，但会增加网络通信压力。
2. **吞吐量 vs 延迟的权衡**：
   - **高吞吐低延迟**（如 API 服务）：选择高带宽 GPU（H200、B200）+ 分离式架构，通过 Decode 侧高 TP 提高并发
   - **低吞吐低延迟**（如实时对话）：优先选择 TTFT（首词延迟，见 [11.5.1 节](11.5_best_practices.md)），采用投机解码或张量并行对 Prefill 加速，Decode 可用低配置 GPU
...
NVIDIA 在 CES 2026 宣布了**Rubin 架构**（代号 Vera Rubin），这是 Blackwell 之后的下一个重大架构演进；2026 年 5 月 31 日的官方公告说的是 Vera Rubin **正在爬产**（“ramping into full production”），并在 Availability 段写明量产出货自 2026 年秋季开始。爬产不等于已交付，更不等同于任一云区域已有即时库存，采购仍需核对具体系统、实例和交付区域。关键特性包括：
